...
public class RegistrarEquipamentoController implements Initializable {
    ...
    private void configurarValidacoes() {
        // Validação de números decimais para medidas
        pesoField.textProperty().addListener((obs, old, novo) -> {
            if (!novo.matches("\\d*(\\.\\d*)?")) {
                pesoField.setText(old);
            }
        });

        larguraField.textProperty().addListener((obs, old, novo) -> {
            if (!novo.matches("\\d*(\\.\\d*)?")) {
                larguraField.setText(old);
            }
        });

        comprimentoField.textProperty().addListener((obs, old, novo) -> {
            if (!novo.matches("\\d*(\\.\\d*)?")) {
                comprimentoField.setText(old);
            }
        });
    }
    ...
    private Equipamento criarEquipamento() {
        ...
        equipamento.setDescricao(descricaoField.getText());
    ...
    private boolean validarCampos() {
        ...
        } else if (equipamentoService.existePorId(codigoField.getText())) {
